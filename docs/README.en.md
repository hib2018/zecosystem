...
- `skills/zintent-interpret/`: generate an external Draft in the human's interaction language from a natural-language request and prepare the `intents/` and `draft/` files needed before running `zintent`.
...
```text
Human Request
  -> zintent-interpret: create a Draft in the human's interaction language and prepare <project>/intents + <project>/draft
...
- Draft item statements use the human's interaction language and remain individually reviewable.
